/**
 * QPLogic — pure functions for ICQA Quality Portals.
 * No DOM, no side effects. UMD module so it works in popup and editor pages.
 */
(function (global, factory) {
  if (typeof module === "object" && module.exports) {
    module.exports = factory();
  } else {
    global.QPLogic = factory();
  }
})(typeof window !== "undefined" ? window : globalThis, function () {

  // ─── Default templates ───────────────────────────────────────────────────
  // These ship with the extension. Users can override via the editor.
  // # = category (tab), ## = QP name, {{token}} = form field.
  // Ron: update the wording here to match real Walmart ICQA verbiage!

  const DEFAULT_TEMPLATES = `# Picking

## Missing / Wrong UPC / Incorrect Product
...
## Mixed Item Receipt
TOTE #{{tote}} | Item No. {{itemNumber}} ({{description}})
Multiple items were received into the same tote: ({{item1Qty}} units) of Item No. {{item1Number}} ({{item1Description}}) and ({{item2Qty}} units) of Item No. {{item2Number}} ({{item2Description}}). All items were received as Item No. {{itemNumber}} ({{description}}). Associate failed to verify all UPCs prior to receipt.
Qty in Error: {{qtyInError}} – {{receiverId}}
...
`;
  ...
  /**
   * Normalize {{tokens}} inside a template body before storing.
   * Allows non-coders to write {{my field}} and have it just work.
   *   {{my field}}       -> {{my_field}}
   *   {{  associate id}} -> {{associate_id}}
   *   {{validToken}}     -> {{validToken}} (unchanged)
   */
  function normalizeTokensInBody(body) {
    return body.replace(/\{\{\s*([^}]+?)\s*\}\}/g, (_, inner) => {
      const normalized = inner.trim().replace(/\s+/g, '_');
      return '{{' + normalized + '}}';
    });
  }
  ...
  /**
   * Parse a markdown blob into a structured array of categories and templates.
   *
   * Rules:
   *   # Heading  → new category (creates a tab in the UI)
   *   ## Heading → new QP template within the current category
   *   ---        → ignored (cosmetic separator)
   *   All other lines → template body text
   *
   * @param {string} blob
   * @returns {{ name: string, templates: { name: string, body: string }[] }[]}
   */
  function parseTemplateBlob(blob) {
    ...
    function flushTemplate() {
      if (!currentTemplate || !currentCategory) return;
      currentTemplate.body = normalizeTokensInBody(bodyLines.join("\n").trim());
    ...
    for (const line of lines) {
      if (/^-{3,}\s*$/.test(line)) continue; // skip ---

      const h1 = line.match(/^#\s+(.+)$/);
      if (h1) {
        flushCategory();
        currentCategory = { name: h1[1].trim(), templates: [] };
        continue;
      }
      ...
      if (h2) {
        flushTemplate();
        if (currentCategory) {
          currentTemplate = { name: h2[1].trim(), body: "" };
        }
        continue;
      }
  ...
  /**
   * Extract unique {{token}} names from a template body, in order of first appearance.
   * Valid tokens: letters, digits, underscores. No spaces.
   *
   * @param {string} body
   * @returns {string[]}
   */
  ...
  /**
   * Convert a camelCase or snake_case token into a human-readable label.
   * Examples:
  ...
  /**
   * Fill a template body with user-provided values.
   * Safe: output is plain text only — always set via textContent, never innerHTML.
   *
   * @param {string} body
   * @param {Record<string, string>} values  key → replacement text
   * @returns {string}
   */
  function generateOutput(body, values) {
    let output = body;
